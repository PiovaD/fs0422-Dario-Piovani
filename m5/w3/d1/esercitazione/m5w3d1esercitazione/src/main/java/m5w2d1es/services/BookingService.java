...
public class BookingService {
	...
	public Booking update(Long id, Booking booking) {
		Optional<Booking> bookingResult = br.findById(id);

		if (bookingResult.isPresent()) {
			Booking bookingUpdate = bookingResult.get();
			bookingUpdate.setValidity(booking.getValidity());
			bookingUpdate.setWorkstation(booking.getWorkstation());
			bookingUpdate.setUser(booking.getUser());
			br.save(bookingUpdate);
			return bookingUpdate;
		} else {
			throw new BookingException("Elemento non aggiornato");
		}
