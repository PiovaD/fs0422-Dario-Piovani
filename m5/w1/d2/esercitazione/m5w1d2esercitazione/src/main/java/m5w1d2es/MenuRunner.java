...
public class MenuRunner implements CommandLineRunner{
	...
	private void valorizzaOrdine(Order order) {
		...
		Pizza pizza = ctx.getBean(PizzaMargherita.class);
		Pizza pizzaTopping1 = (Pizza) ctx.getBean("toppingOnions", pizza);
		pizzaTopping1 = (Pizza) ctx.getBean("toppingHam", pizzaTopping1);
		OrderItem oi1 = new OrderItem(pizzaTopping1, "");
		order.addMenuItem(oi1);

		Pizza pizza2 = ctx.getBean(Diavola.class);
	...
	public void stampaOrdine(Order order) {
		...
		log.info("Numero Tavolo: {}", order.getTable().getId());
		log.info("Stato ordine: {}", order.getStatus());
		log.info("");
		for (OrderItem item : order.getList()) {
			log.info("{} |-> Prezzo: {}", item.getItem().getName(),item.getItem().getPrice());
		}
		log.info("");
